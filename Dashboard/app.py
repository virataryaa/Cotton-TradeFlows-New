...
with tab1:
    ...
    _rep_default_sf = (
        [r for r in ["Brazil"] if r in _all_reps_sf] if _is_exports
        else [r for r in ["United States", "United States of America"] if r in _all_reps_sf]
    ) or _all_reps_sf
            ...
            with st.expander("Monthly Breakdown — Top 15 Partners + Rest of World", expanded=False):
                ...
                if _mo_cy_opts:
                    ...
                    def _style_monthly_ctn(df):
                        ...
                        styled = styled.set_table_styles([{"selector":"th","props":[("text-align","center"),("font-size","8px"),("font-weight","600")]}])
                        return styled
                    st.dataframe(_style_monthly_ctn(_mo_tbl), use_container_width=True, height=38 * (len(_mo_tbl.index) + 1) + 10)
                else:
